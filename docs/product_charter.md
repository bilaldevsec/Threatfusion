...
| Alert handling and correlation | The versioned Attack-only `AlertCandidate` and local SQLite boundary carry stable registered-offline ingestion identity, source and model decision into durable idempotent storage. The bounded correlation interface remains to define. | Duplicate candidate insertion is idempotent; conflicting identity or persistence failure is visible. Correlation failure behavior remains to define and verify. |
