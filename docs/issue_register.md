...
| TF-008 | P0 / critical | **Demo/acceptance gap; terminating producer-inference worker implemented through 2026-09-22.** The [producer protocol](producer_admission_transport_policy.md) joins TLS identity, full admission, rate/concurrency, replay claim, durable audit, spawned whole-batch registered preparation/Random Forest inference, parent-owned Attack-only persistence, completion audit, replay completion and response-write attempt in fixed order. Exact 30-second record and 300-second request production deadlines are parent-enforced. Worker crash, hang, silent exit, malformed/missing result, child exception or cleanup failure is terminated/joined and creates no partial alert; failures are sanitized. Exact completed replay starts no worker. Real IPv4-loopback TLS 1.3 reaches the frozen RF through the worker. Validation passed 16 focused worker, 491 related and 13 FYP-focused tests; the one complete backend run passed 944 tests with four existing TF-005 warnings and no failures/skips. Terminal-message hardening then passed the final focused matrix and real worker smoke; the full suite was not repeated under its exactly-once constraint. A serving loop, correlation, explanation and dashboard remain absent. Azure remains disabled pending a live-source/identity contract. | Core product, demo readiness, usefulness claims | Add the next bounded product contract without bypassing the worker; a serving lifecycle or correlation/analyst-state contract remains to choose. | One authenticated supported input reaches the dashboard with reconciled identity/status; authentication/admission failures have zero predictor/repository calls; failures are visible; optional services can be off; replay/duplicates are idempotent; bounded resource evidence is recorded. | open |
| TF-009 | P1 / high | **Reliability evidence gap; terminating worker failure/timeout cleanup and real-transport success evidence implemented through 2026-09-22.** Regressions cover success, crash, hang/forced termination, record/request deadlines, malformed/missing result, cleanup failure, exact PID/process-group cleanup, sanitized responses, invalid-request zero execution/persistence, no partial alert after a prior Attack result, exact cached replay, real-mTLS frozen-RF worker execution, production defaults and a later healthy request. The measured counts are recorded under TF-008 and the producer policy. Existing replay/finalization fatality remains separate. Audit export/retention/rotation, backup recovery, optional-service outages, a serving lifecycle and operational resource limits remain unverified. | Core workflow, demo reliability, deployment claims | Preserve terminating isolation and add broader service/resource/recovery evidence under a separately bounded milestone. | The charter's malformed/partial-input, mismatch, offline-service, restart/replay, idempotency, interruption, timeout, concurrency/rate/body/queue, database backup/recovery and measured-resource cases pass end to end. | open |
