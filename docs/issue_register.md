...
| TF-008 | P0 / critical | **Demo/acceptance gap; corrected synchronous producer orchestrator and bounded real-mTLS integration implemented through 2026-09-14.** The [producer protocol](producer_admission_transport_policy.md) joins TLS identity, full admission, rate/concurrency, replay claim, durable audit, whole-batch registered preparation, Random Forest inference, Attack-only persistence, completion audit, replay completion and response-write attempt in fixed order. The six reproduced review findings are corrected: later invalid registered evidence causes zero prediction/persistence; ambiguous finalization poisons the generation and is failure-audited; lease failure cannot bypass connection close; and v1/v2 response validation is strict and sanitized. Eight real IPv4-loopback TLS 1.3 cases include all three later-reference rejection modes through the actual registered reader/adapter and one available frozen-artifact functional smoke. The focused slice passes 422 tests and the complete suite passes 852 tests with four existing TF-005 warnings and no skips. A serving/terminating-worker boundary, correlation, explanation and dashboard remain absent. Azure remains disabled pending a live-source/identity contract. | Core product, demo readiness, usefulness claims | Implement a bounded worker-process/service wrapper before correlation/analyst-state work; preserve the real loopback integration evidence. | One authenticated supported input reaches the dashboard with reconciled identity/status; authentication/admission failures have zero predictor/repository calls; failures are visible; optional services can be off; replay/duplicates are idempotent; bounded resource evidence is recorded. | open |
| TF-009 | P1 / high | **Reliability evidence gap; corrected synchronous failure/replay/recovery and real-transport evidence implemented through 2026-09-14.** Regressions cover full registered preflight before prediction, exact completion/replay/response/release ordering, audit-before-inference, request-bound failure auditing after an actual committed-alert ambiguity, fatal finalization timeout, explicit recovery to `outcome_unknown`, completed cached reopen retry with zero repeated effects, strict response versions, and nested connection cleanup. Eight real-mTLS cases and the 422-test focused slice pass; the complete suite passes 852 tests with four existing TF-005 warnings and no skips. The synchronous boundary still cannot terminate a blocked predictor; audit export/retention/rotation, backup recovery, optional-service outages and operational resource limits remain unverified. | Core workflow, demo reliability, deployment claims | Add a terminating worker process and real TLS integrated interruption/timeout/recovery tests for the authenticated vertical slice. | The charter's malformed/partial-input, mismatch, offline-service, restart/replay, idempotency, interruption, timeout, concurrency/rate/body/queue, database backup/recovery and measured-resource cases pass end to end. | open |
