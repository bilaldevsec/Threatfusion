...
| TF-008 | P0 / critical | **Demo/acceptance gap; terminating producer serving lifecycle implemented through 2026-09-23.** The [producer protocol](producer_admission_transport_policy.md) now adds explicit startup/readiness, two fixed no-queue handlers around the one-request orchestrator, existing gate-enforced one-request execution, graceful bounded shutdown and clean restart. Successive real-mTLS requests, authenticated overlap/busy response, exact replay after restart, worker failure/timeout and full local cleanup are tested without changing the spawned frozen-RF path or exact 30/300-second deadlines. A short controlled-inference resource sample is recorded, but is not endurance or frozen-RF latency evidence. Correlation, explanation and dashboard remain absent. Azure remains disabled pending a live-source/identity contract. | Core product, demo readiness, usefulness claims | Define the bounded correlation/analyst-state contract without bypassing the worker/lifecycle; separately add an operator entry point and operational/endurance evidence when authorized. | One authenticated supported input reaches the dashboard with reconciled identity/status; authentication/admission failures have zero predictor/repository calls; failures are visible; optional services can be off; replay/duplicates are idempotent; bounded resource evidence is recorded. | open |
| TF-009 | P1 / high | **Reliability evidence gap; bounded serving cleanup/restart evidence implemented through 2026-09-23.** Regressions now add startup/readiness, graceful drain, successive/overlapping requests, exact replay after clean restart, idle-accept audit silence, worker failure/timeout and listener/thread/lease/worker/process-group/partial-alert/SQLite-sidecar cleanup. The measured short-workload counts are recorded under TF-008 and the producer policy. This does not establish endurance, frozen operational limits, service-manager/signal behavior, database backup/recovery, audit export/retention/rotation, or optional-service outages. | Core workflow, demo reliability, deployment claims | Preserve terminating isolation and add the remaining operational resource, service-entry, backup/recovery and degradation evidence under separately bounded milestones. | The charter's malformed/partial-input, mismatch, offline-service, restart/replay, idempotency, interruption, timeout, concurrency/rate/body/queue, database backup/recovery and measured-resource cases pass end to end. | open |
