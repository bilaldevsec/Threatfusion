...
| TF-008 | P0 / critical | **Demo/acceptance gap; terminating producer lifecycle and bounded per-alert analyst state implemented through 2026-09-23.** The [producer protocol](producer_admission_transport_policy.md) provides explicit readiness, fixed no-queue handling, graceful shutdown/restart and tested cleanup while preserving exact 30/300-second deadlines. The [analyst-state contract](alert_correlation_analyst_state.md) adds authorized bounded list/detail reads and `new → in_review → {closed \| escalated}` with idempotent, append-only transitions in separate storage. Retained alerts contain no defensible relationship key, so distinct-alert correlation is explicitly not implemented and no incident/window claim is made. Explanation, authenticated external analyst API and dashboard remain absent. Azure remains disabled pending a live-source/identity contract. | Core product, demo readiness, usefulness claims | Define an authenticated analyst API/view model over the implemented list/detail/state boundary; separately obtain a reviewed retained relationship key before proposing correlation, and add the remaining operator/operational evidence when authorized. | One authenticated supported input reaches the dashboard with reconciled identity/status; authentication/admission failures have zero predictor/repository calls; failures are visible; optional services can be off; replay/duplicates are idempotent; bounded resource evidence is recorded. | open |
